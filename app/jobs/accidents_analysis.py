...
from utilities.spark import get_spark_session, stop_spark 
from readers.load import load_csv
from writer.write import write_to_parquet
...
class AccidentDataAnalysis:
    ...
    def count_accidents_with_greater_than_2_males_death(self) -> int:
        """
        Crash counts where male count killed is greater than 2
        :return: dataframe count
        """
        df_primary_person_male = self.df_primary_person \
                .filter(col('PRSN_DEATH_TIME').isNotNull() & (col('PRSN_GNDR_ID') == "MALE"))

        male_crash_count = df_primary_person_male.groupBy('CRASH_ID') \
            .agg(count('CRASH_ID').alias('MALE_DEATH_COUNT'))
            
        write_to_parquet(male_crash_count, self.result_path.get("Analysis1"))
        
        return male_crash_count.filter(col('MALE_DEATH_COUNT')>2).count()
    ...
    def count_vehicle_with_valid_licence_and_hr(self) -> int:
        """
        Number of Vehicles with driver having valid licences involved 
        in hit and run
    ...
    def get_state_with_highest_accidents_with_no_females_involved(self) -> str:
        ...
        df_crash_with_females = self.df_primary_person\
    ...
    def get_top3to5_companies_contributing_to_injuries(self)-> list:
        """
        finds top 3rd-5th manufacturers contributing to large number of 
        injuries and deaths
        :return: list
        """
    ...
    def get_top_ethnic_group_for_bodystyle(self)-> DataFrame:
        """
        ...
        df_units_cleaned = self.df_units \
                .filter(~col('VEH_BODY_STYL_ID').isin(undefined_body_values)) \
        ...
        df_result = df_units_cleaned \
                .join(df_primary_person_cleaned, on=['CRASH_ID','UNIT_NBR'], how='inner') \
    ...
    def get_top_5_vehicle_brand_with_speedcharges(self)-> list:
        ...
        with speeding related offences, has licensed Drivers, uses top 10 used 
        vehicle colours and has car licensed with the Top 25 states with highest 
        number of offences
        :return List of Vehicle brands
        """
        df_units_cleaned = self.df_units \
                .select('CRASH_ID','UNIT_NBR','VIN','VEH_LIC_STATE_ID','VEH_COLOR_ID','VEH_MAKE_ID') \
                .distinct()
        ...
        df_pp_licenced = self.df_primary_person \
            .filter(col('DRVR_LIC_TYPE_ID').isin(['DRIVER LICENSE', 'COMMERCIAL DRIVER LIC.']))

        df_speed_charges = self.df_charges \
                .filter(col('CHARGE').contains('SPEED') | col('CHARGE').contains('FAST')) \
                .select('CRASH_ID', 'UNIT_NBR') \
                .distinct()

        df_result = df_units_cleaned \
                .join(df_speed_charges, ['CRASH_ID', 'UNIT_NBR'], 'inner') \
    ...
    def getResult(self)-> None:
        ...
        print('Analyis 1 result-> ', \
              self.count_accidents_with_greater_than_2_males_death() )
